...
| 文件 | 内容 |
|---|---|
| [LKUS 拦截策略公开摘要 2026-09-10](docs-strategy/LKUS_拦截策略_公开摘要_20260910.md) | 4 条拦截策略 + 1 项阈值调整 + 2 项风控之外措施的评估结果、回测证据、上线流程与回滚原则 |

> 该文件为内部《拦截策略明细》的**脱敏公开摘要**。规则原文、生效阈值、区号名单、
> 告警阈值、已知绕过路径与上线时间点等可被用于规避防护的内容均未公开，
> 需通过内部渠道获取原始文档。详见该文件第八节。
